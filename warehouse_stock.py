...
class WarehouseStock(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # ── Main container — grid so both columns resize with window ──
        main_container = tk.Frame(self, bg='#F5F6FA')
        main_container.pack(fill="both", expand=True, padx=12, pady=8)

        # Column 0 = form (fixed), column 1 = list (expands)
        ...
        # ── Action Buttons (stacked vertically so they never overflow) ─
        ...
        # Initially disable Update/Delete
        ...
        # Make list_frame's interior expand properly
        list_frame.columnconfigure(0, weight=1)
        list_frame.rowconfigure(1, weight=1)   # row 1 = treeview
