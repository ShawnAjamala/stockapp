...
class AuthWindow:
    def __init__(self, root):
        self.root = root
        self.root.title("Storage System - Login")
    ...
    def create_widgets(self):
        ...
        tk.Label(header, text="SUPERMARKET STORAGE SYSTEM", 
                font=("Arial", 14, "bold"), bg='#2c3e50', fg='white').pack(pady=35)
    ...
    def create_login_tab(self, parent):
       #Creates Login form
        ...
        # Email
        tk.Label(container, text="Email:", bg='white').pack(anchor='w')
        self.login_email = tk.Entry(container, width=30)
        ...
        self.login_password = tk.Entry(container, show="*", width=30)
        ...
        tk.Button(container, text="LOGIN", command=self.login,
                 bg='#3498db', fg='white', font=("Arial", 10, "bold")).pack(fill="x")
    ...
    def create_register_tab(self, parent):
        ...
        # Full Name
        tk.Label(container, text="Full Name:", bg='white').pack(anchor='w')
        self.reg_name = tk.Entry(container, width=30)
        self.reg_name.pack(fill="x", pady=(0, 10))
        
        # Email
        tk.Label(container, text="Email:", bg='white').pack(anchor='w')
        self.reg_email = tk.Entry(container, width=30)
        self.reg_email.pack(fill="x", pady=(0, 10))
        
        # Role
        tk.Label(container, text="Role:", bg='white').pack(anchor='w')
        self.reg_role = ttk.Combobox(container, values=["supermarket", "warehouse"])
        self.reg_role.pack(fill="x", pady=(0, 10))
        
        # Role Password
        tk.Label(container, text="Role Password:", bg='white').pack(anchor='w')
        self.reg_password = tk.Entry(container, show="*", width=30)
        self.reg_password.pack(fill="x", pady=(0, 20))
        
        # Register button
        tk.Button(container, text="REGISTER", command=self.register,
                 bg='#2ecc71', fg='white', font=("Arial", 10, "bold")).pack(fill="x")
    ...
    def open_dashboard(self, user):
       #redirects to the specific dashboard depending on ones role
        ...
        # Header
        header = tk.Frame(dashboard, bg='#2c3e50', height=80)
        header.pack(fill="x")
        tk.Label(header, text=f"{user['role'].upper()} ADMIN DASHBOARD", 
                font=("Arial", 14, "bold"), bg='#2c3e50', fg='white').pack(pady=25)
        ...
        if user['role'] == 'supermarket':
            tk.Label(content, text="\nSupermarket Functions:", 
                    font=("Arial", 10, "bold"), bg='#ecf0f1').pack(pady=10)
        ...
        tk.Button(content, text="LOGOUT", command=logout,
                 bg='#e74c3c', fg='white', width=15, font=("Arial", 10, "bold")).pack(pady=20)
